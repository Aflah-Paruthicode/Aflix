  ...
  return from == "browse" ? (
    <div className="inline-flex w-full md:gap-24 justify-between items-center bg-gradient-to-b from-black z-[999999999]  px-16 max-md:px-5 absolute">
      ...
      <div className="mt-2 flex gap-5 items-center text-white font-semibold max-md:z-[9999999]">
        ...
        <svg className="max-md:hidden" xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#FFFFFF">
        ...
        {isDropDownEnable && (
          <div className="absolute top-20 right-10 p-3 bg-[#000000c6] border w-[270px] border-[#ffffff39] rounded-sm max-md:z-[499999]">
            {usersInTheAc.map((ele, ind) => {
              return (
                <div className="flex gap-3 m-2 text-white hover:underline hover:decoration-red-600 hover:decoration-2 underline-offset-4 cursor-pointer">
                  {ele.icon && <img className="w-10 rounded-sm h-10 object-cover" src={ele.icon} alt="" />}
                  {ele.svg && (
                    <svg
            ...
            <p className="text-center hover:underline hover:decoration-red-600 hover:decoration-2 underline-offset-4 cursor-pointer" onClick={handleSignOut}>
              Sign Out Of Netflix
            </p>
          </div>
        )}
      ...
      {from == "login" ? (
        ...
        <div className="inline-flex w-full gap-24 max-md:gap-3 max-md:mt-2 justify-around items-center">
          ...
          <div className="mt-2">
            <select
              className={` ${
                from == "register" ? "bg-white mr-4 text-[#000000e3]" : "text-white mr-4 bg-[#0d0d0d78]"
            ...
            <button onClick={() => navigate("/login")} className="py-1 px-4 bg-red-600 text-white font-semibold rounded-sm cursor-pointer">
              Sign in
            </button>
          </div>
        </div>
